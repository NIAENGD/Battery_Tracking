...
Battery Tracking is a Windows App SDK (WinUI 3) solution targeting Windows-on-ARM (WoA) devices. It combines a headless
collector, a command-line control surface, and developer tooling to investigate battery and subsystem power consumption on
Snapdragon-class hardware.

## Highlights by phase

- ✅ **Phase 1 — Inception:** Verified availability of core Windows power telemetry on WoA hardware, implemented a prototype
  `AggregateBatterySensor`, established SQLite persistence, delivered a WinUI 3 dashboard shell, and captured findings in
  `docs/inception/inception-summary.md`.
- ✅ **Phase 2 — Core collector:** Added a `CpuPerformanceSensor`, a verb-based CLI (`BatteryTracker.Cli`), and an integration test
  harness that validates session persistence with mocked adapters.
- ✅ **Phase 3 — Extended telemetry:** Introduces GPU, display, and wireless adapters, enriches diagnostics with a scripted
  self-test, and delivers a bootstrap `.bat` that can build, run, and validate the collector end-to-end.
...
## Getting started on Windows on ARM
...
3. To run the WinUI dashboard, set `BatteryTracker.App` as the startup project and press **F5**.
4. Telemetry sessions persist to `%LOCALAPPDATA%\BatteryTracker\battery-tracker.db` by default.

## Command-line usage

The CLI defaults to `%LOCALAPPDATA%\BatteryTracker` for data, logs, and session state. All commands accept `--data-directory`
...
# Run a 20-second diagnostics capture that verifies core + extended telemetry and writes a text summary.
batterytracker selftest --output C:\logs\batterytracker-selftest.log
```

## Batch bootstrapper

`BatteryTracker.bat` in the repository root provides one-click flows for WoA testers:

```
BatteryTracker.bat build       # Publish the CLI for win-arm64 (self-contained)
BatteryTracker.bat start ...   # Forward arguments to the CLI start command
BatteryTracker.bat stop        # Forward stop command
BatteryTracker.bat status      # Inspect the most recent collector state
BatteryTracker.bat test        # Build (if needed), run the 20s diagnostics self-test, emit a summary log
```

The `test` command orchestrates a 20-second diagnostics session that exercises battery, CPU, GPU, display, and wireless
sensors. After completion, a text summary is written to `artifacts\diagnostics\telemetry-selftest-<timestamp>.log` alongside
the raw SQLite database and collector logs for further inspection.

## Extended telemetry adapters

Phase 3 introduces three additional adapters that complement the existing battery and CPU coverage:

- **GPU engine sensor** — Uses Windows Performance Counters to track GPU engine utilization and adapter power estimates.
- **Display brightness sensor** — Queries `WmiMonitorBrightness` to capture panel brightness and estimate panel wattage.
- **Wireless throughput sensor** — Samples wireless NIC byte counters to infer RX/TX throughput and link utilization.

These adapters stream into the same ingestion pipeline and surface in diagnostic summaries produced by the CLI self-test.
...
- Provide a Windows-on-ARM (WoA) native desktop utility that can capture, persist, and visualize battery and power-related
  telemetry for the system and its major subsystems (CPU, GPU, display, storage, wireless radios, etc.).
- Allow users to launch the application through a `.bat` bootstrapper that exposes simple commands (`start`, `stop`, `status`,
  `export`).
- Maintain continuous logging when tracking is enabled, minimizing overhead (<5% CPU usage, <100 MB RAM) and automatically
  buffering data during transient disconnects.
- Export human-readable PDF reports that contain timelines, summary statistics, and contextual metadata for a selected time
  window.
- Ship as an arm64-native, self-contained package installable without external dependencies beyond the Windows 11-on-ARM
  baseline.
...
1. User runs `BatteryTracker.bat`.
...
- Include `BatteryTracker.bat` and configuration files in distributable ZIP.
...
- `BatteryTracker.bat` — entry point script with inline help.

---
This plan balances native Windows-on-ARM compatibility, reliable access to system telemetry, and maintainable reporting workflows
while leaving room for vendor-specific enhancements as Qualcomm expands tooling support.
